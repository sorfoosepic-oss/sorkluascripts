...
createCorner(BarBG, UDim.new(1, 0))
...
function CreateGameSelector()
	...
	createCorner(Selector, UDim.new(0, 14))
	...
	local function CreateGameButton(name, enabled, order)
		...
		if enabled then
			Btn.MouseButton1Click:Connect(function()
				-- cerrar selector
				tween(Selector, 0.3, { BackgroundTransparency = 1 })
				task.wait(0.32)
				safeDestroy(Selector)
				CreateMainUI(name)
			end)
			...
			Btn.MouseLeave:Connect(function()
				tween(Btn, 0.15, { BackgroundColor3 = CONFIG.Card })
			end)
		else
...
-- =====================================================
-- MÓDULOS POR JUEGO (base para que luego avances)
-- =====================================================
local GameModules = {}

GameModules["Crecer Pollo"] = function(ctx)
	-- ctx = { TogglesState = ..., UI = ... }
	-- Aquí luego conectas lógica PERMITIDA.
	-- Por ahora dejamos placeholders.
	for toggleName, val in pairs(ctx.TogglesState) do
		-- prints opcionales:
		-- print("[Crecer Pollo] Toggle:", toggleName, val)
	end
end
...
function CreateMainUI(gameName)
	...
	-- Estado de toggles (para que la UI sea “real”)
	local TogglesState = {}

	-- Headshot bolita flotante
	...
	Sidebar.Parent = Main
	createCorner(Sidebar, UDim.new(0, 12))
	...
	-- =========================
	-- Crear Toggle (con callbacks reales)
	-- =========================
	local function CreateToggle(parent, toggleName, default, onChanged)
		-- default boolean
		TogglesState[toggleName] = default and true or false
		...
		createCorner(holder, UDim.new(0, 8))
		...
		toggle.MouseButton1Click:Connect(function()
			local newState = not TogglesState[toggleName]
			TogglesState[toggleName] = newState

			tween(toggle, 0.2, { BackgroundColor3 = newState and CONFIG.ToggleOn or CONFIG.ToggleOff })
			tween(circle, 0.2, {
	...
	for _, cat in ipairs(categories) do
		...
		btn.MouseButton1Click:Connect(function()
			-- actualizar highlight botones
			for _, v in ipairs(CatList:GetChildren()) do
				if v:IsA("TextButton") then
					local isActive = (v == btn)
					v.BackgroundTransparency = isActive and 0 or 1
					v.BackgroundColor3 = isActive and Color3.fromRGB(40, 35, 55) or Color3.fromRGB(0, 0, 0)
					v.TextColor3 = isActive and CONFIG.Text or CONFIG.TextDim
				end
			end
	...
	-- =========================
	-- Botón cerrar: CIERRE COMPLETO (ScreenGui Destroy)
	-- =========================
	...
	createCorner(CloseBtn, UDim.new(0, 7))
	...
	-- Toggle bolita: mostrar/ocultar panel
	Toggle.MouseButton1Click:Connect(function()
	...
	-- Ejecutar módulo por juego (base)
	if GameModules[gameName] then
	...
	print("✅ Sorkscripts | " .. gameName .. " cargado")
end
